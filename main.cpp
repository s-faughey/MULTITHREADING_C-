...
void task(int h, int y, int w, int samps, Vec r, Vec cx, Vec cy, Ray cam, Vec *c) {
...
int main(int argc, char *argv[]) {
	int w = 512, h = 384, samps = argc == 2 ? atoi(argv[1]) / 4 : 2; // # samples
	Ray cam(Vec(50, 52, 295.6), Vec(0, -0.042612, -1).norm()); // cam pos, dir
	Vec cx = Vec(w*.5135 / h), cy = (cx%cam.d).norm()*.5135, *c = new Vec[w*h];
	//auto r = make_shared<Vec>();
	Vec r;
...
#pragma omp parallel for schedule(dynamic, 1) private(r)       // OpenMP
	for (int y = 0; y<h; y++) {                       // Loop over image rows
		...
		threads.push_back(thread(task, h, y, w, samps, r, cx, cy, cam, c));
	}
	for (auto &t : threads) {
		t.join();
	}
	FILE *stream;
	errno_t err;
	err = fopen_s(&stream, "image.ppm", "w");         // Write image to PPM file.
	fprintf(stream, "P3\n%d %d\n%d\n", w, h, 255);
	for (int i = 0; i < w*h; i++)
		fprintf(stream, "%d %d %d ", toInt(c[i].x), toInt(c[i].y), toInt(c[i].z));
}